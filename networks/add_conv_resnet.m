function [lgraph, input_name, output_name] = add_conv_resnet(lgraph, block_name, input_size, kvargs)
% [lgraph, input_name, output_name] = add_conv_resnet(lgraph, block_name, input_size)
% [lgraph, input_name, output_name] = add_conv_resnet(__, Name, Value)
%
% Adds convolutional resnet layers to a layer graph
%
% The input and output layers are left unconnected and must be connected after
% add_conv_resnet is called.  The names of the input and output layers are returned
...
% output_name - Name of block output layer (to be connected by caller)
  ...
  conv_1_name = sprintf('%s_conv1', block_name);
  conv_act_name = sprintf('%s_tanh_conv1', block_name);
  conv_2_name = sprintf('%s_conv2', block_name);
  ...
  lgraph = addLayers(lgraph, ...
    convolution1dLayer(1, input_size, 'Name', input_name, ...
      'Padding', 'same', ...
      'Stride', 1));

  % Add convolution layers and connect the input
  lgraph = addLayers(lgraph, [...
    convolution1dLayer(filter_size, input_size, 'Name', conv_1_name, ...
      'Padding', 'same', ...
      'Stride', 1)
    tanhLayer('Name', conv_act_name)
    convolution1dLayer(filter_size, input_size, 'Name', conv_2_name, ...
      'Padding', 'same', ...
      'Stride', 1)
  ]);
  lgraph = connectLayers(lgraph, input_name, conv_1_name);

  % Add projection layer and connect the input
  lgraph = addLayers(lgraph, ...
    convolution1dLayer(1, input_size, 'Name', projection_name, ...
      'Padding', 'same', ...
      'Stride', 1));
  ...
  lgraph = connectLayers(lgraph, conv_2_name, sprintf('%s/in2', addition_name));

  % Add output layer
  lgraph = addLayers(lgraph, tanhLayer('Name', output_name));
