function [losses, grads, training_params] = evaluate_resnet(model, batch, training_params)
  losses = [];
  grads = [];
...
%% Initialize parameters
  ...
  dim_B = 2;
  dim_T = 3;
  size_C = size(batch, dim_C);
  size_B = size(batch, dim_B);
  size_T = size(batch, dim_T);
...
%% Encode input
  encoder_output = forward(model.encoder, batch);
...
%% Sample latent space and reconstruct input
  ...
  for i = 1:monte_carlo_reps
    ...
    recon_loss = recon_loss + mse(decoder_output, batch);
...
% Total loss is the sum of reconstruction loss and KL divergence loss
  ...
  [grads.decoder, grads.encoder] = ...
    dlgradient(losses.total_loss, ...
      model.decoder.Learnables, model.encoder.Learnables);
